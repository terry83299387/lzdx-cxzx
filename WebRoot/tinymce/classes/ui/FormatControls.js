...
/**
 * Internal class containing all TinyMCE specific control types such as format
 * listboxes, fontlist boxes, toolbar buttons etc.
 * 
 * @class tinymce.ui.FormatControls
 */
define("tinymce/ui/FormatControls", ["tinymce/ui/Factory",
				"tinymce/ui/Control", "tinymce/ui/FloatPanel",
				"tinymce/util/Tools", "tinymce/EditorManager"], function(
				Factory, Control, FloatPanel, Tools, EditorManager) {
			var each = Tools.each;
			...
			function getPreviewCss(format) {
				...
				// Get parent container font size so we can compute px values
				// out of em/% for older IE:s
				parentFontSize = dom.getStyle(editor.getBody(), 'fontSize',
						true);
				parentFontSize = /px$/.test(parentFontSize) ? parseInt(
						parentFontSize, 10) : 0;
				...
				each(previewStyles.split(' '), function(name) {
					...
					if (name == 'color') {
						// Ignore black since it's the default color, not the
						// nicest fix
						// TODO: Fix this by detecting runtime style
					...
					// Old IE won't calculate the font size so we need to do
					// that manually
					if (name == 'font-size') {
						if (/em|%$/.test(value)) {
							if (parentFontSize === 0) {
			...
			EditorManager.on('AddEditor', function(e) {
				...
				function createFormatMenu() {
					...
					var menu = createMenu(editor.settings.style_formats
							|| defaultStyleFormats);
				...
				each({
							...
							editor.addButton(name, {
								...
								onPostRender : function() {
									...
									// TODO: Fix this
									if (editor.formatter) {
										editor.formatter.formatChanged(name,
												function(state) {
													self.active(state);
												});
									} else {
				...
				function toggleFormat(fmt) {
					...
					if (fmt) {
						EditorManager.activeEditor.execCommand(
								'mceToggleFormat', false, fmt);
					}
				}
				...
				Factory.add('styleselect', function(settings) {
							...
							/*
							 * menu.push({text: '-'}); menu.push({ text: 'Remove
							 * formatting', icon: 'removeformat', onclick:
							 * function() { editor.execCommand('RemoveFormat'); }
							 * });
							 */
				...
				Factory.add('formatselect', function(settings) {
							var items = [], blocks = createFormats('Paragraph=p;'
											+ 'Address=address;'
											+ 'Pre=pre;'
											+ 'Header 1=h1;'
											+ 'Header 2=h2;'
				...
				Factory.add('fontselect', function(settings) {
					var items = [], fonts = createFormats('黑体=SimHei;'
									+ '宋体=SimSun;'
									+ '新宋体=NSimSun;'
									+ '仿宋=FangSong;'
									+ '楷体=KaiTi;'
				...
				Factory.add('fontsizeselect', function(settings) {
					...
					each('8pt 10pt 12pt 14pt 18pt 24pt 36pt'.split(' '),
							function(item) {
								items.push({
											text : item,
											format : item
