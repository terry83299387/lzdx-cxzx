	...
	init : function() {
		...
		this.startBtn = new Ext.Button({
			...
			template : new Ext.Template(
					...
					'<td class="ux-startbutton-left"><i>&#160;</i></td><td class="ux-startbutton-center"><em unselectable="on"><button class="x-btn-text" type="{1}" style="height:28px;">{0}</button></em></td><td class="ux-startbutton-right"><i>&#160;</i></td>',
					"</tr></tbody></table>")
		});


		var width = Ext.get('ux-startbutton').getWidth() + 10;
		var width1 = Ext.get('ux-tray-panel').getWidth() + 10;
		...
		var cuBox = new Ext.BoxComponent({
			el : "ux-taskbar-currentuser",
			id : 'TaskBarCurrentUser',
			minWidth : width,
			region : 'east',
			width : width
				// split : true
		...
		this.tbPanel = new Ext.ux.TaskButtonsPanel({
					...
					id : 'TaskBarButtons',
					width : Ext.lib.Dom.getViewWidth() - width - width1,
					region : 'center'
				});

		var panelWrap = new Ext.Container({
					el : 'ux-taskbar-panel-wrap',
					items : [this.quickStartPanel, this.tbPanel, this.trayPanel],
					layout : 'border',
					region : 'center'
				});
			...
			onRender : function() {
				...
				this.cmenu = new Ext.menu.Menu({
							items : [{
										...
										text : i18n.tb_minimize,
										handler : this.win.minimize,
										scope : this.win
									}, {
										text : i18n.tb_maximize,
...
Ext.ux.TrayPanel = Ext.extend(Ext.BoxComponent, {
			initComponent : function() {
				Ext.ux.TrayPanel.superclass.initComponent.call(this);
